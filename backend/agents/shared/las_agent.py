...
try:
    ...
    raise SystemExit("psutil is required for las_agent.py") from exc


CONFIG_PATH = Path(os.getenv("LAS_CONFIG") or "/etc/las/agent.conf")
AGENT_VERSION = "4.1.1"
...
def collect_interfaces(primary_ip: str | None) -> list[Dict[str, Any]]:
...
def collect_payload() -> Dict[str, Any]:
    ...
    return {
        ...
        "interfaces": collect_interfaces(primary_ip),
    }
