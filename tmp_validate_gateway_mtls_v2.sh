#!/bin/bash
set -euo pipefail
WORKDIR=/tmp/las-mtls-validate2
rm -rf "$WORKDIR"
...
text = '\n'.join(line for line in text.splitlines() if 'lasmtls' not in line) + '\n'
hosts.write_text(text, encoding='utf-8')
PY
  if [ -f "$WORKDIR/gateway/pid" ]; then
    kill "$(cat "$WORKDIR/gateway/pid")" >/dev/null 2>&1 || true
  fi
}
trap cleanup EXIT
...
import json
from pathlib import Path
base = Path('/tmp/las-mtls-validate2/gateway')
...
echo "OK:no-cert-blocked"
curl -fsSL --cert "$WORKDIR/gateway/mtls-client.pem" --key "$WORKDIR/gateway/mtls-client-key.pem" --cacert "$WORKDIR/gateway/mtls-ca.pem" https://localhost:${GATEWAY_PORT}/health

echo "127.0.0.1 api.soservices.com.br # lasmtls" >> /etc/hosts
...
import json
from pathlib import Path
payload = json.loads(Path('/tmp/las-mtls-validate2/agent-gw/bootstrap.json').read_text())
for name in ['agent-gw', 'agent-direct']:
    base = Path('/tmp/las-mtls-validate2') / name
    (base / 'mtls-ca.pem').write_text(payload['ca_pem'], encoding='ascii')
    (base / 'mtls-client.pem').write_text(payload['client_cert_pem'], encoding='ascii')
    (base / 'mtls-client-key.pem').write_text(payload['client_key_pem'], encoding='ascii')
PY
